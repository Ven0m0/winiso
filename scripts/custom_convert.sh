#!/bin/bash
# shellcheck disable=SC1091,SC2016,SC2034,SC2066,SC2129,SC2162,SC2166,SC2181,SC2219,SC2261
...
TARGET_EDITION="${TARGET_EDITION:-ProfessionalWorkstation}"
FALLBACK_EDITION="${FALLBACK_EDITION:-Professional}"
...
for metadata in "${metadataFiles[@]}"; do
  ...
  if [[ "$scanEdition" == "$TARGET_EDITION" ]]; then
    ...
    break
  elif [[ "$scanEdition" == "$FALLBACK_EDITION" ]] && [[ -z "$fallbackMetadata" ]]; then
    fallbackMetadata="$metadata"
  fi
done

# Use target if found, else fallback
if [[ -n "$targetMetadata" ]]; then
  metadataFiles=("$targetMetadata")
  echo -e "${infoColor}""Found target edition: ${TARGET_EDITION}""$resetColor"
elif [[ -n "$fallbackMetadata" ]]; then
  metadataFiles=("$fallbackMetadata")
  echo -e "${infoColor}""Target edition ${TARGET_EDITION} not found, using fallback: ${FALLBACK_EDITION}""$resetColor"
else
