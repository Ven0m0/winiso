...
def select_editions(build_info):
    ...
    for filename, file_info in files.items():
        if filename.endswith('.esd') and any(ed in filename.lower() for ed in
                ['professional', 'enterprise', 'home', 'core', 'education']):
...
def download_build(build_id, output_dir, edition_filter=None):
    ...
    print(f"{Colors.BOLD}This may take a while depending on your connection...{Colors.RESET}\n")

    aria2_cmd = [
        'aria2c',
        '--input-file', str(aria2_input),
        '--dir', str(output_path),
    ...
    try:
        ...
        downloaded = len([f for f in output_path.glob('*') if f.is_file() and f.name != '.gitkeep'])
...
def interactive_mode(output_dir):
    ...
    while True:
        try:
            choice = input(f"{Colors.BOLD}Select build number [1-{len(builds)}] or 'q' to quit:{Colors.RESET} ").strip()
...
For more information, visit: https://uupdump.net
    ...
    parser.add_argument('-b', '--build-id',
                       help='Specific build ID to download')

    parser.add_argument('-l', '--list',
                       action='store_true',
                       help='List latest builds and exit')
